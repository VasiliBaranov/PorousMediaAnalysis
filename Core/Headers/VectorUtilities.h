...
#define CONTAINER_VALUE_TYPE(TContainer)typename TContainer::value_type
    ...
    public:
        ...
        template<class TContainer, class TContainerMultiplier, class TContainerResult>
        static void MultiplyByValue(const TContainer& vector, const TContainerMultiplier& multiplier, TContainerResult* result)
        {
        ...
        template<class TContainerTo, class TContainerValue, class TContainerResult>
        static void Min(const TContainerTo& x, const TContainerValue& y, TContainerResult* result)
        {
            const std::size_t size = x.size();
            TContainerResult& resultRef = *result;
            for (std::size_t i = 0; i < size; ++i)
            {
                resultRef[i] = std::min(x[i], y[i]);
            }
        }

        template<class TContainerTo, class TContainerValue, class TContainerResult>
        static void Max(const TContainerTo& x, const TContainerValue& y, TContainerResult* result)
        {
            const std::size_t size = x.size();
            TContainerResult& resultRef = *result;
            for (std::size_t i = 0; i < size; ++i)
            {
                resultRef[i] = std::max(x[i], y[i]);
            }
        }
        ...
        template<class TContainer, class TContainerResult>
        static void Ceil(const TContainer& vector, TContainerResult* result)
        {
            const std::size_t size = vector.size();
            TContainerResult& resultRef = *result;
            for (std::size_t i = 0; i < size; ++i)
            {
                resultRef[i] = std::ceil(vector[i]);
            }
        }
        ...
        static void Round(const TContainer& vector, TContainerResult* result)
        {
            ...
            for (std::size_t i = 0; i < size; ++i)
            {
                resultRef[i] = Math::Round<CONTAINER_VALUE_TYPE(TContainerResult)>(vector[i]);
            }
        }
        ...
        {
            typedef CONTAINER_VALUE_TYPE(TContainer) TItemType;
            return GetProductGeneric<TContainer, TItemType>(vector);
        }

        template<class TContainer, class TResult>
        static TResult GetProductGeneric(const TContainer& vector)
        {
            typedef CONTAINER_VALUE_TYPE(TContainer) TItemType;
            TResult product = TResult(1);
            const std::size_t size = vector.size();
            for (std::size_t i = 0; i < size; ++i)
            {
                product *= vector[i];
        ...
        template<class TContainer>
        static FLOAT_TYPE GetMean(const TContainer& vector)
        {
            return Sum(vector) / vector.size();
        }

        template<class TContainer>
        static FLOAT_TYPE GetVariance(const TContainer& vector)
        {
            if (vector.size() <= 1)
            {
                return 0;
            ...
            typedef CONTAINER_VALUE_TYPE(TContainer) TItemType;
            TItemType mean = GetMean(vector);

            std::vector<TItemType> squares(vector.size());
            SubtractValue(vector, mean, &squares);

            Multiply(squares, squares, &squares);
            TItemType squaresSum = Sum(squares);

            FLOAT_TYPE variance = static_cast<FLOAT_TYPE>(squaresSum) / (vector.size() - 1); // unbiased estimate
            return variance;
        }

        template<class TContainer>
        static FLOAT_TYPE GetStd(const TContainer& vector)
        {
            FLOAT_TYPE variance = GetVariance(vector);
