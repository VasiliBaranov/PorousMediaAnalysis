...
namespace Core
{
    ...
    public:
        ...
        static T*** Allocate3DArray(size_t xsize, size_t ysize, size_t zsize)
        {
            ...
            ReindexMemory(xsize, ysize, zsize, grid);
            return grid;
        }

        // You may call this function to make grid represent array of a smaller size,
        // changing the pointer values from grid[i] and grid[0][i].
        // Still, you may then call Free3DArray.
        // This exposes a vulnerability, if new xsize, ysize, and zsize are larger than initial ones.
        // TODO: remove vulnerability (create a class to handle memory, store initial size, do checks).
        template<typename T>
        static void ReindexMemory(size_t xsize, size_t ysize, size_t zsize, T*** grid)
        {
